...
Este es el primer post de `ERRE`. Antes de comenzar a escribir código, creo que es necesario establecer bases sólidas que nos permitan aprovechar R al máximo. En este post hablaremos sobre las generalidades de R y RStudio, y las ventajas de trabajar en este ambiente. Presentaré una introducción al uso de proyectos de RStudio para tener un ambiente de trabajo ordenado, y hablaremos brevemente sobre maneras de ejecutar código. Finalmente, hablaremos sobre el uso de paquetes en R para extender la funcionalidad de R.

# El ambiente de trabajo en RStudio

¿Cuál es la diferencia entre R y RStudio? Esta pregunta me la han hecho varias veces, y creo que es válido aclarar esta duda antes de comenzar con la parte del código. R es un lenguaje de programación con enfoque estadístico. R es el motor que ejecuta las operaciones que el usuario pide. Hay varias formas en las que el usuario puede comunicarse con ese motor. La manera más básica es utilizar R desde la interfaz de usuario de R:

![](/img/2018-06-04-r-y-r-studio/Rgui.JPG)

Aunque esta plataforma nos permite comunicarnos con R y hacer todas las operaciones, no brinda un ambiente de trabajo flexible. Como lo dice Hadley Wickham,

> No se puede hacer *data science* desde una interfaz de usuario [^1].

[^1]: [Video](https://www.youtube.com/watch?v=cpbtcsGE0OA)

Es aquí donde RStuido se vuelve importante. RStudio es un Ambiente de Desarrollo Integrado (IDE, *Integrated Development Environment*) que brinda todas las erramientas dentro de la interfaz de usuario. Esto es muy útil, pues nos evita tener que cambiar de ambientes para hacer diferentes tareas, y podemos integrar todo nuestro proceso de trabajo en un mismo ambiente. Esto permite la replicación de análisis y resultados.
...
Notarás que hay más divisiones o secciones que en el RGui. En RStudio estas secciones reciben el nombre de **páneles**. A medida que avancemos en este post describiremos a detalle cada uno de ellos, pero aquí hay un resumen de las partes principales:

- `Console`: Aquí verás un cursos que parpadea a la derecha de un símbolo `>`. Escribe `2 + 2` y presiona la tecla `Enter`. La computadora responde diciendo `[1] 4`. Esta consola nos sirve para comunicarnos con R. Al pedirle la suma, R respondió diciendo que el resultado es `4`. Por ahora ignoraremos el valor de `[1]` que aparece a la izquierda.
- `Environment`: Este panel mostrará todos los objetos y variables que carguemos. Por ejemplo, en la consola escribe `a <- 2 + 2` y presiona `Enter`. Verás que esta vez R no respondió, pero en nuestro ambiente ahora hay algo llamado `a`, donde R almacenó el resultado de la operación que le pedimos. Si en la consola escribes `a` y presionas `Enter`, R dirá: `[1] 4`.
- `History`: Este pánel te mostrará un historial de las operaciones anteriores. Si haces doble click sobre uno de los renglones, el texto se copiará a la consola para que puedas volver a ejecutar la operación. Desde la consola también puedes acceder a operaciones pasadas. Para esto, debes colocar el cursos en la consola y presionar la tecla que apunta hacia arriba en tu teclado. Cada vez que la presiones regresarás un paso atrás.
- `Files`: Aquí aparecerán los archivos que estén en el directorio de trabajo. En la siguiente sección hablaremos sobre como mantener archivos ordenados mediante el uso de proyectos.
...
RStudio también te permite modificar el ambiente de trabajo, incluyendo diferentes páneles o especificando donde quieres que aparezcan. También es posible modificar los esquemas de colores y el tamaño de fuente. Todas estas características las puedes explorar en la pestaña de `Herramientas`, bajo la sección de `Opciones globales` y `Opciones de proyecto`.

# Investigación reproducible

Es importante que nuestros análisis y resultados puedan ser reproducidosy corroborados. Uno de los grandes avances de la humanidad es la capacidad de comunicar información por medio del lenguaje escrito. Escribir código nos permite comunicar nuestros procedimientos y contar una historia, pues el código no es más que texto. Aunque tiene su propia estructura, el código no es más que texto que puede ser comprendido por computadoras y humanos al mismo tiempo. Por ejemplo, el siguiente código es muy obvio para humanos y computadoras:

```
2 + 2
```

Existen muchos lenguajes de programación, cada uno con sus reglas semánticas y gramaticales. No importa cuan complicado sea el código, la intuición es la misma. Al escribir código generamos una receta o bitácora explícita de nuestros procedimientos, permitiendo que sean replicados. Además, podemos incluir mensajes que sean fáciles de leer por humanos pero que la computadora ignore. En R esto se hace con el uso del símbolo de número o *hashtag*: `#`. Todo el texto a la derecha de ese símbolo será ignorado por R, pero brinda información al humano que esté leyendo el código, por ejemplo:
...
Comentar el código nos ayuda a explicar las operaciones que hacemos. Esto es útil al compartir nuestro código con alguien más, pero también al regresar a código viejo. Más de una vez he regresado a código viejo que, por alguna razón, no comenté y termino perdiendo mucho tiempo identificando lo que hago.

# Proyectos de RStudio

Utilizar proyectos en R es una de las mejores formas de mantener un ambiente de trabajo ordenado. El proyecto mantiene un directorio de trabajo estable, donde mantenemos nuestros archivos asociados. El directorio de trabajo no es más que la localización de todos los archivos. El comando `getwd()` (*get working directory*) te mostrará el directorio de trabajo. Si es la primera vez que abres RStudio o estás fuera de un proyecto,el resultado será:

`[1] "C:/Usuarios/TuUsuario/Documentos"`

Dentro de Documentos yo tengo todos mis archivos, así que trabajar en este nivel podría ser complicado. Por ejemplo, asumamos que tengo dos proyectos de investigación. Uno es sobre peces rojos y otro de pájaros azules. Para cada uno de estos tendré archivos con datos, que mantengo separados en folders con la siguiente estructura:
...
4. En este caso, yo ya tengo folders con mis datos, por lo que seleccionaré la opción de *Existing Directory*, que asociará mi proyecto de RStudio con una directorio existente. Al seleccionar esa opción aparece una nueva ventana, pidiéndome que indice el directorio al cual quiero asociar mir poyecto:
...
¿Tantos pasos y todo se ve igual? No, al poner más atención verás algunas diferencias. Por ejemplo, la esquina superior derecha ahora dice `PecesRojos`, indicándonos que estamos en ese proyecto. La pestaña de `Files` mostrará la carpeta donde tenemos los datos, y un projecto de R llamado `PecesRojos.Rproj`. Además, el pánel con el historial de operaciones está vacío. Lo más importante es que si ahora ejecutamos el comando para obtener el directorio (`getwd()`) obtenemos:

`[1] "C:/Usuarios/TuUsuario/Documentos/Proyectos/PecesRojos"`

Estto puede parecer trivial, pero tener un directorio asociado a nuestro proyecto nos permite cargar y guardar datos, figuras y tablas más fácilmente. Estos temás se cubrirán más adelante en detalle. Por ahora, procura usar proyectos de RStudio para administrár mejor tu trabajo y mantener todo más ordenado.

# Scripts y documentos de `rmakrdown`

Como vimos anteriormente, podemos ejecutar comandos directamente en la consola. Sin embargo, la mayoría de las veces realizaremos varias operaciones simultáneamente, y necesitamos tener tódo el código en un documento. Esto nos ahorra tener que escribir todas las operaciones cada vez, pero también nos da un archivo que podemos archivar para futuras referencias o compartir. Aquí exploraremos dos formas de hacerlo.
...
Para crear un documento de `rmarkdown` seguirás el mismo camino que para crear un *script*. Sin embargo, seleccionarás la opción que dice *R Markdown*. Al hacerlo, deberás ver una ventana así:
...
Ahora, para aprovechar al máximo el documento, ve a la parte superior y da click en la bola de estambre que dice *Knit* (tejer). R te pedirá que indiques un nombre del archivo. Al crear el documento especificamos el **Título** del archivo, que no necesariamente es el nombre del archivo. Piensa que el título podría ser *"El Viejo y el Mar"*, y el nombre del archivo `libro_Hemingway.Rmd`. Guarda tu documento dentro de tu carpeta del proyecto; a mi me gusta tener por lo menos carpetas distintas para `datos`, `scripts` y `docs`. Al guardar el documento, R ejecutará todos los *chunks* y generará un documento de `*.html` que contiene el texto, código, y resultados del código. 
